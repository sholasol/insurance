...
<html>
    ...
    include 'nav.php';
    $id=$_GET['id'];
    $tp=$con->query("SELECT type FROM product_type WHERE type_id='$id'");
    $ro=$tp->fetch_array();
    $typ = $ro['type'];
    ...
    if(isset($_POST['submit'])){
        ...
        echo  " <script>alert('$e'); window.location='Etype.php?id=$id'</script>";
	}
        
        else{
           $nam = check_input($_POST["name"]);
           
           $pr=$con->query("UPDATE product_type SET type='$nam' WHERE type_id='$id'");
