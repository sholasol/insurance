...
<html>
    ...
    $id=$_GET['id'];
    $co=$con->query("SELECT * FROM users, complain WHERE users.user_id=complain.userID AND complain.id='$id'");
    $r=$co->fetch_array();
    ...
    if(isset($_POST['submit'])){
        ...
        $e="Policy number cannot be empty"; 
        echo  " <script>alert('$e'); window.location='Vcomplain.php'</script>";
	}
        elseif(empty($_POST['comp'])){
        $e="Complain field cannot be empty'"; 
        echo  " <script>alert('$e'); window.location='Vcomplain.php'</script>";
	}else{
           ...
           $rem = check_input($_POST["remark"]);
           
           
            
            $clm=$con->query("UPDATE complain SET  remark='$rem', status='Resolved', resolved=now(), resolved_by='$user_id' WHERE id='$id' ");
            if($clm){
                $act=$con->query("INSERT INTO activities SET userID='$uid', activity='Treated Customer complain', query='Update', role='4', created=now() ");
                                                            ...
                                                            <div class="row">
                                                                ...
                                                                <div class="form-group col-md-12">
                                                                    ...
                                                                    <select class="form-control"  name="policy" id="policy" required>
                                                                        <?php 
                                                                        echo "<option >".$polp. "</option>";
                                                                        ?>
                                                                    </select>
                                                                </div>
                                                                <div class="form-group col-md-12">
                                                                    <label for="exampleInputEmail">Complain</label>
                                                                    <textarea class="form-control" name="comp" id="exampleInputEmail" rows="4" required><?php echo $comp; ?></textarea>
                                                                </div>
                                                                <div class="form-group col-md-12">
                                                                    <label for="exampleInputEmail">Remark</label>
                                                                    <textarea class="form-control" name="remark" id="exampleInputEmail" rows="4" required></textarea>
                                                                </div>
                                                                <div class="form-group col-md-12">
