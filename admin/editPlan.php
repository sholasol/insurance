...
<html>
    ...
    include 'nav.php';
    $id=$_GET['id'];
    $prID=$con->query("SELECT * FROM plan_setup WHERE plan_id='$id'");
    $rp=$prID->fetch_array();
    $prod=$rp['product_id'];
...
if(isset($_POST['submit'])){
        ...
        echo  " <script>alert('$e'); window.location='editPlan.php?id=$id'</script>";
	}
        if(empty($_POST['name'])){
        ...
        echo  " <script>alert('$e'); window.location='editPlan.php?id=$id'</script>";
           ...
           $pr=$con->query("UPDATE plan_setup SET name='$plan', premium='$prem', sum_assured='$sum' WHERE plan_id='$id'");
           
           if($pr){
               $e="Plan has been Successfully updated! ."; 
                echo  " <script>alert('$e'); window.location='editPlan.php?id=$id'</script>";
           }else{
            $e="Processing error."; 
            echo  " <script>alert('$e'); window.location='editPlan.php?id=$id'</script>";
                            ...
                            <div class="panel panel-white">
                                <div class="panel-body">
                                    <div id="rootwizard">
                                        ...
                                        <?php
                                        $p=$con->query("SELECT * FROM product_setup WHERE product_id='$prod'");
                                        $r=$p->fetch_array();
                                        ?>
                                            ...
                                            <div class="tab-content">
                                                <div class="tab-pane active fade in" id="tab1">
                                                    <div class="row m-b-lg">
                                                        <div class="col-md-10">
                                                            <div class="row">
                                                                <input type="hidden"name="name"  value="<?php echo $r['product_name']; ?>" />
                                                                <div class="form-group col-md-6">
                                                                    <label for="exampleInputName">Product Name</label>
                                                                    <input type="text" class="form-control" disabled name="nam" id="exampleInputName" value="<?php echo $r['product_name']; ?>" required>
                                                                </div>
                                                                <div class="form-group  col-md-6">
                                                                    <label for="exampleInputName2">Plan</label>
                                                                    <input type="text" class="form-control" name="plan" id="exampleInputExpiration" value="<?php echo $rp['name']; ?>" required>
                                                                </div>
                                                                <div class="form-group  col-md-6">
                                                                    <label for="exampleInputName2">Premium</label>
                                                                    <input type="number" class="form-control col-md-6" name="premium" id="premium" value="<?php echo $rp['premium']; ?>" required>
                                                                </div>
                                                                <div class="form-group  col-md-6">
                                                                    <label for="exampleInputName2">Sum Assured</label>
                                                                    <input type="number" class="form-control col-md-6" name="sum" id="sum" value="<?php echo $rp['sum_assured']; ?>" required>
                                                                </div>
