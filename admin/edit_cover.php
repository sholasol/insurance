...
$p=$con->query("SELECT * FROM cover WHERE cover_id='$id'");
$r=$p->fetch_array();
$cover = $r['cover'];
$amt =$r['amount'];
$pla = $r['plan_id'];

$pl=$con->query("SELECT * FROM plan_setup WHERE plan_id='$pla'");
$ro=$pl->fetch_array();
$plan = $ro['name'];
$prod = $ro['product_id'];
...
if(isset($_POST['submit'])){
        if(empty($_POST['name'])){
        $e="Cover name is mandatory!"; 
        echo  " <script>alert('$e'); window.location='edit_cover.php&id=$id'</script>";
	}
        
        else{
            $cid = check_input($_POST["covid"]);
           $cov = check_input($_POST["cover"]);
           $amount = check_input($_POST["amount"]);
           
           $pr=$con->query("UPDATE cover SET cover='$cov', amount='$amount' WHERE cover_id='$cid'");
           
           if($pr){
               $e="Cover has been successfully updated."; 
                echo  " <script>alert('$e'); window.location='managecover.php?id=$pla&prod=$prod'</script>";
           }else{
            $e="Processing error."; 
            echo  " <script>alert('$e'); window.location='edit_cover.php?id=$id'</script>";
            }
                ...
                <div class="page-title">
                    ...
                    <div class="page-breadcrumb">
                        <ol class="breadcrumb">
                           ...
                           <!-- <li><a href="manageplan.php?id=<?php echo $id ?>">Plan</a></li>-->
                                ...
                                <div class="panel-body">
                                    <div id="rootwizard">
                                        ...
                                        <form id="wizardForm" method="post" action="" name="FormData">
                                            <input type="hidden" name="covid" value="<?php echo $id; ?>"/>
                                                        ...
                                                        <div class="col-md-10">
                                                            ...
                                                            <div class="row">
                                                                <div class="form-group  col-md-6 ">
                                                                    ...
                                                                    <input type="text" class="form-control" name="cover" id="exampleInputName" value="<?php echo $cover; ?>" required>
                                                                </div>
                                                                <div class="form-group  col-md-6 ">
                                                                    <label for="exampleInputName2">Cover Amount</label>
                                                                    <input type="text" class="form-control" name="amount" id="exampleInputName" value="<?php echo $amt; ?>" required>
                                                                </div>
                                                                <div class="form-group col-md-12"> 
