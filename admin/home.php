                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table id="example" class="display table" style="width: 100%; cellspacing: 0;">
                                        ...
                                        <tbody>
                                            <?php 
                                            $cus=$con->query("SELECT * FROM users, transaction WHERE users.user_id = transaction.user_id AND transaction.status !='Renewed' AND transaction.insurance_id='$i_id' LIMIT 10");
                                            $counter =0;
                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table id="example" class="table">
                                    ...
                                    <tbody>
                                            <?php 
                                            $cus=$con->query("SELECT * FROM users, claims WHERE users.user_id = claims.user_id AND claims.insurance_id='$i_id' AND claims.status='Pending' LIMIT 10");
                                            $counter =0;
                                            while($row=$cus->fetch_array()){
                                                $id=$row['user_id'];
                                            ?>
...
<div class="page-footer">
	...
	<?php 
        //Sales and Expenses
        $sl=$con->query("SELECT sum(premium) AS premium, sum(commission_value) AS com, created FROM commission WHERE insurance_id='$i_id' GROUP BY created");
        //Premium
        $json1 =[]; // Premium
        $json2 =[]; // Commission
        $json3 =[]; //Created
        ...
        //Sales and Premium
        $sp=$con->query("SELECT sum(premium) AS premium, MONTH(start_date) AS month, count(policy) AS policy FROM transaction WHERE status='Active' AND insurance_id='$i_id' GROUP BY MONTH(start_date) ");
        //Premium
        $json5 =[]; // Premium
