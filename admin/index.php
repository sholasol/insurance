...
if (isset($_SESSION["id"])){
    ...
    $ins_comp=$con->query("SELECT insurance_id FROM iuser WHERE user_id='$uid'");
    $ra=$ins_comp->fetch_array();
    $i_id=$ra['insurance_id'];
    
    
    $cc=$con->query("SELECT company_name FROM insurance_company WHERE company_id='$i_id'");
    $ar=$cc->fetch_array();
}else{
...
elseif (isset($_GET['manageplan'])){
    include_once "manageplan.php";
}
elseif (isset($_GET['managecover'])){
    include_once "managecover.php";
}
elseif (isset($_GET['editcover'])){
    include_once "edit_cover.php";
}
