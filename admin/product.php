...
<html>
    ...
    if(isset($_POST['submit'])){
        ...
        echo  " <script>alert('$e'); window.location='product.php'</script>";
           ...
           $nam = check_input($_POST["name"]);
           $type = check_input($_POST["type"]);
           $cat = check_input($_POST["cat"]);
           
           $chk=$con->query("SELECT count(product_id) AS count, product_id FROM product_setup WHERE product_name='$nam' AND insurance_id='$inss'");
           $rr=$chk->fetch_array();
           $num=$rr['count'];
           $pid= $rr['product_id'];
            
           if($num > 0){
               $e="This product $nam Already exists."; 
                echo  " <script>alert('$e'); window.location='manageProd.php'</script>";
           }else{
               $pr=$con->query("INSERT INTO product_setup SET product_name='$nam', insurance_id='$inss', status='Active', category='$cat', start_date=now(), type='$type'");
               
               $prID=$con->query("SELECT  product_id FROM product_setup WHERE product_name='$nam' AND insurance_id='$inss'");
               $ro=$prID->fetch_array();
               $proID=$ro['product_id'];
               
           if($pr){
               $e="Operation successful. Please create a plan for this Product."; 
                echo  " <script>alert('$e'); window.location='plan.php?id=$proID'</script>";
           }else{
            $e="Processing error."; 
            echo  " <script>alert('$e'); window.location='product.php'</script>";
            }
                                                        ...
                                                        <div class="col-md-10">
                                                            <div class="row">
                                                                <div class="form-group col-md-12">
                                                                    ...
                                                                    <input type="text" class="form-control" name="name" id="exampleInputName" placeholder="Product Name" required>
                                                                </div>
                                                                <div class="form-group  col-md-6">
                                                                    ...
                                                                    <select name="type" class="form-control" required>
                                                                        ...
                                                                        <?php 
                                                                        $pro= $con->query("SELECT * FROM product_type WHERE insurance_id='$i_id'");
                                                                ...
                                                                <div class="form-group  col-md-6">
                                                                    ...
                                                                    <select name="cat" class="form-control" required>
                                                                        <option value=""> Please Select</option>
                                                                        <?php 
                                                                        $pro= $con->query("SELECT * FROM category WHERE insurance_id='$i_id'");
                                                                        while($rr=$pro->fetch_array()){
                                                                            $ct=$rr['category'];
