                ...
                <div class="col-md-6">
                    <div class="panel panel-white">
                        <div class="panel-heading clearfix">
                            <h4 class="panel-title">Recent Transactions<?php echo $uid ?></h4>
                        </div>
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table id="example" class="display table" style="width: 100%; cellspacing: 0;">
                                        ...
                                        <tbody>
                                            <?php 
                                            $cus=$con->query("SELECT * FROM users, transaction WHERE users.user_id = transaction.user_id AND transaction.partner_id='$uid' LIMIT 10");
                                            $counter =0;
                                            while($row=$cus->fetch_array()){
                                                $id=$row['user_id'];
                                                
                                                $cnt=$con->query("SELECT count(transaction_id) AS transaction, count(policy) AS policy, sum(premium) AS premium FROM transaction WHERE user_id='$id' AND partner_id='$uid' AND status !='Renewed' AND status !='Terminated' ");
                                                $ro=$cnt->fetch_array();
                                            ?>
                                            <tr>
                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table id="example" class="table">
                                    ...
                                    <tbody>
                                            <?php 
                                            $cus=$con->query("SELECT * FROM  claims WHERE partner_id= '$uid'  LIMIT 10");
                                            $counter =0;
                                            while($ro=$cus->fetch_array()){
                                                $id=$ro['user_id'];
                                                
                                                $cnt=$con->query("SELECT firstname, lastname FROM users WHERE user_id='$id'");
                                                $row=$cnt->fetch_array();
                                            ?>
                                            <tr>
                                                <td><?php echo ++$counter;?> </td>
