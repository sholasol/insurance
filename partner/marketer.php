...
<html>
   ...
   if(isset($_POST['submit'])){
        ...
        $pcode= 'MI/MK/'.random_num(5);
        
        $chk=$con->query("SELECT count(user_id) AS count, user_id FROM users WHERE email='$email' ");
        //$rr=$chk->num_rows;
        $row=$chk->fetch_array();
        $mkt=$row['user_id'];
        $count=$row['count'];
        if($count < 1){
            ...
            if($reg){
                ...
                $mark=$con->query("INSERT INTO pmarketer SET partner_id ='$user', user_id= '$idd', code='$pcode'");
                if($mark){
                    $e="Marketer's registration is successful"; 
                    echo  " <script>alert('$e'); window.location='manage_marketer.php' </script>";
                }
                else{
                  $e="Unable to map marketer to the partner"; 
                  echo  " <script>alert('$e'); window.location='marketer.php'</script>";  
                }
            }else{
            ...
            $ckMk=$con->query("SELECT count(id) AS count FROM pmarketer WHERE partner_id ='$user' AND user_id= '$mkt'");
            $rd=$ckMk->fetch_array();
            $mCount=$rd['count'];
            
            if($mCount < 1){
                $inn=$con->query("INSERT INTO pmarketer SET partner_id ='$user', user_id= '$mkt', code='$pcode'");
                if($inn){
                    $e="Marketer's registration is successful"; 
                    echo  " <script>alert('$e'); window.location='manage_marketer.php' </script>";
                }else{
                ...
                $e="This marketer already exists"; 
                echo  " <script>alert('$e'); window.location='marketer.php'</script>";
            }
        }
                                                        ...
                                                        <div class="col-md-10">
                                                            <div class="row">
                                                                ...
                                                                <div class="form-group col-md-6">
                                                                    ...
                                                                    <input type="number" class="form-control" name="phone" id="exampleInputPhone" placeholder="Telephone Number" required />
                                                                </div>
                                                                <div class="form-group col-md-6">
