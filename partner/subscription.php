...
<div class="page-inner">
    ...
    <div class="row">
                            ...
                            <div class="panel panel-white">
                                ...
                                <div class="panel-body">
                                   <div class="table-responsive">
                                       <form name="bulk_action_form" action="sub.php" method="post" onsubmit="return subscribeConfirm();"/>   
                                    <table id="example" class="display table" style="width: 100%; cellspacing: 0;">
                                        <thead>
                                            <tr>
                                                <th width="3%"></th>
                                                <th>Insurance Name</th>
                                                <th width="12%">Total Subscription</th>
                                        ...
                                        <tbody>
                                            <?php
                                            //$ins=$con->query("SELECT  insurance_company.company_name, insurance_company.phone ,insurance_company.address, ipartner.insurance_id FROM insurance_company, ipartner WHERE insurance_company.company_id = ipartner.insurance_id  AND ipartner.partner_id='$uid'");
                                            $ins=$con->query("SELECT * FROM insurance_company, ipartner WHERE insurance_company.company_id=ipartner.insurance_id AND ipartner.partner_id='$uid'");
                                            while($row=$ins->fetch_array()){
                                            ?>
                                            <tr>
                                                <td><input type="checkbox" value="<?php echo $row['company_id']; ?>" name="rid[]"></td>
        ...
        function subscribeConfirm(){
            var result = confirm("Are you sure to subscribe for the selected companies?");
            if(result){
                return true;
            }else{
                return false;
